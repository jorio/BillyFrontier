...
#define	EmVector3D_Ptr_Dot(_nx,_ny,_nz,_v)								\
			((_nx * _v->x) + (_ny * _v->y) + (_nz * _v->z))
...
const static OGLVector2D	xax = {1,0};
OGLVector2D					aim;
...
/******************* OGL:  POINT 2D DISTANCE TO LINE ********************/

float OGLPoint2D_DistanceToLine(OGLPoint2D *end0, OGLPoint2D *end1, OGLPoint2D *point)
{
float	a,b,c, dist;
	...
	a = end0->y - end1->y; 
	b = end1->x - end0->x; 
	c = end0->x * end1->y - end1->x * end0->y; 


	dist = a * point->x + b * point->y + c; 
	dist = sqrt(dist);
...
/******************** OGL: VECTOR 3D RNADOM OFFSET **********************/
//
// This function will randomly offset the trajectory of a vector
//

void OGLVector3D_RandomOffset(OGLVector3D *v, float offsetFactor, OGLVector3D *outVec)
{
const OGLVector3D	yup = {0,1,0};
OGLVector3D			xaxis, yaxis;
float				off;

	OGLVector3D_Cross(v, &yup, &xaxis);			// calc perpendicular x-axis
	OGLVector3D_Cross(v, &xaxis, &yaxis);		// calc perp y-axis
	
			/* OFFSET ALONG Y PERP */
			
	off = RandomFloat2() * offsetFactor;
	yaxis.x *= off;
	yaxis.y *= off;
	yaxis.z *= off;
	...
	outVec->x = v->x + yaxis.x + xaxis.x;
	outVec->y = v->y + yaxis.y + xaxis.y;
	outVec->z = v->z + yaxis.z + xaxis.z;
	FastNormalizeVector(outVec->x, outVec->y, outVec->z, outVec);
...
/******************** OGL: COMPUTE TRIANGLE PLANE EQUATION *********************/
...
const OGLPoint3D 	*p1, *p2, *p3;
	...
	/*
	 *	The next two are an expansion of:
	 *	EiTrianglePoints_ComputeNormal(&trianglePoints, structSize, &planeEquation->normal);
	 */
	...
	/* EiPoint3D_CrossProductTri(p1, p2, p3, planeEquation->normal); */
	v0x = p1->x - p2->x;
	v0y = p1->y - p2->y;
	v0z = p1->z - p2->z;
	 ...
	 *	nDotO = EiVector3D_Normalize(&peNormal, &peNormal);
	 */
	{
		float	length, oneOverLength;
	...
	planeEquation->normal.x = nx;
	planeEquation->normal.y = ny;
	planeEquation->normal.z = nz;

	planeEquation->constant = - EmVector3D_Ptr_Dot(nx, ny, nz, p1);
