...
const char gBackupURLString[3][256] =					// this is compared with the copy below to see if pirates hacked it
{
	{"http://www.pangeasoft.net/billy/files/updatedata"},
	{"http://www.briangreenstone.com/updatedata/billy/updatedata"},
	{"http://homepage.mac.com/pangea/updatedata/billy/updatedata"},
};
...
const char urlString[3][256] =
{
	...
	for (i = 0; i < 3; i++)
	{
		if (strcmp(gBackupURLString[i], urlString[i]) != 0)			// are both strings identical or different?
			goto invalid_url;	
	}
	...
	for (i = 0; i < 3; i++)								// try primary URL and then secondary, tertiary if that fails
	{
		gHTTPDataIndex = 0;
...
invalid_url:				
		...
		/***********/
		//
		// If gets here then all URL's failed - possibly from pirate firewall stuff
