		...
		switch(eventType)
		{
			...
			case	ANIMEVENT_TYPE_PLAYSOUND:
					if (!gDisableAnimSounds)
					{
						...
						{
							case	0:
									PlayEffect_Parms3D(EFFECT_SPURS2, &theNode->Coord, NORMAL_CHANNEL_RATE + (MyRandomLong() & 0x3fff), .3);
									break;

							case	1:
									PlayEffect_Parms3D(EFFECT_WALKERCRASH, &theNode->Coord, NORMAL_CHANNEL_RATE, 1.0);
									break;


							case	2:
									PlayEffect_Parms3D(EFFECT_WALKERFOOTSTEP, &theNode->Coord, NORMAL_CHANNEL_RATE, 1.0);
									break;
